...
function _get_automatic_code_review_json {
	find /tmp -maxdepth 1 -mtime +3 -name "pr-*-*.json" -delete
	find /tmp -maxdepth 1 -mtime +3 -name "pr-*-*.raw" -delete

	rm --force /tmp/pr-${pr_number}-* /tmp/pr-${pr_number}.*
...
Output ONLY valid JSON: {"chance": <0-100>, "violations": ["one description per violation"]} where chance is your confidence that Brian Chan would close this PR for these violations.'
	local response
	...
	echo "${response}" > "/tmp/pr-${pr_number}-${model}.raw"
