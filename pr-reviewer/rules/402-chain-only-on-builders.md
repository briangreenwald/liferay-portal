...
Do not chain method calls unless the receiver is a builder being assembled, or a fluent type whose methods are designed to be chained — `*Builder`, `Stream`, `Optional`, `Instant`, `JSONArray`, `JSONObject`, and the rest of the list maintained by `ChainingCheck` in `modules/util/source-formatter`. When a call returns an object outside that set and you would immediately call another method on that result, assign the result to a named local variable and call the next method on the variable instead.
